...
/// each, and `dispatch_message` tries their `handle` in the order listed, stopping
...
/// at each call site and flows into every mixin's `handle` -- the same
...
struct MessageDispatcher : Mixins... {
    LRESULT dispatch_message(this auto& self, UINT msg, WPARAM wparam, LPARAM lparam) {
        ...
        ((result = handle<Mixins>(self, msg, wparam, lparam)) || ...);
        if (result)
            return *result;
...
private:
    // One mixin's handle, called on the full object. Exists only because MSVC
    // rejects expanding the pack inside `self.Mixins::handle` in the fold
    // (C7515); a template argument is a position it can expand.
    template <typename Mixin>
    static std::optional<LRESULT> handle(auto& self, UINT msg, WPARAM wparam, LPARAM lparam) {
        return self.Mixin::handle(msg, wparam, lparam);
    }
};
